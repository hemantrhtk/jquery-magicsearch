  ...
  (function(factory) {
      ...
      formatParse = function formatParse(format, data) {
        ...
        for (var _i = 0; _i < fields.length; _i++) {
          format = format.replace(
            SEPARATOR + fields[_i] + SEPARATOR,
            data[fields[_i]] ? data[fields[_i]] : "error"
          );
        }
    ...
    MagicSearch.prototype = {
      init: function init() {
        ...
        window.MagicSearch.index += 1; //you must do those things before ajax
        ...
        if ($input.parent().attr("data-belong") !== "magicsearch") {
          var $wrapper = $(
            '<div class="'.concat(
              doms.wrapper + (this.options.skin ? " " + this.options.skin : ""),
        ...
        var inputDisplay = $input.css("display"); //init magicsearch wrapper
        ...
        $magicsearch_wrapper.attr("data-index", window.MagicSearch.index); //init input
        ...
        if ($magicsearch_wrapper.find("." + doms.box).length === 0) {
          $magicsearch_wrapper.append(
            '<div class="'.concat(doms.box, '"></div>')
          );
        }
        ...
        if (this.options.hidden) {
          ...
          var $hidden = $(
            '<input class="'
              .concat(doms.hidden, '" type="hidden" value="')
              .concat(ids, '">')
          );
        ...
        if (this.options.dropdownBtn) {
          var $arrow = $('<div class="'.concat(doms.arrow, '"><i></i></div>'));
        ...
        if (this.options.type == "ajax") {
          var $loading = $(
            '<div class="'.concat(doms.loading, '"><div></div></div>')
          );
          ...
          var ajaxOptions = {
            ...
            error: function error() {
              console.error(
                "magicsearch: Error with xhr.Index: " + window.MagicSearch.index
              );
            },
      ...
      initAfterAjax: function initAfterAjax() {
        ...
        if (isString(this.options.fields)) {
          this.options.fields = [
            this.options.fields === "" ? this.options.id : this.options.fields
          ];
        ...
        if (this.options.multiple) {
          ...
          if (
            ...
            var $items = $('<div class="'.concat(doms.items, '"></div>'));
            $items.css({
        ...
        if (this.options.multiple) {
          ...
          if (ids !== "") {
            ...
            var format = this.options.inputFormat
      ...
      deleteData: function deleteData(id) {
        ...
        for (var i = 0; i < delIdArr.length; i++) {
          $magicsearch_wrapper
            .find(".".concat(doms.item, '[data-id="').concat(delIdArr[i], '"]'))
            .fadeOut(400, delCallBack);
          idArr.splice(idArr.indexOf(delIdArr[i]), 1);
        }
  
        setTimeout(function() {
          var maxLineItem = parseInt(
            (styles.maxWidth -
      ...
      searchData: function searchData(isAll, isScroll) {
        ...
        var options = this.options,
          ...
          htmlStr = "",
          data = [],
          isAppendHtml = true; //if is not scroll,clean all items first
        ...
        if (isAll) {
          ...
          data = dataJson.slice(0); //skip selected data
        ...
        if (data.length === 0) {
          ...
          htmlStr = '<span class="no-result">'.concat(noResult, "</span>");
        } else {
          //delete empty input
          ...
          _inputVals = _inputVals.concat(tempArr); //locate highlight chars
          ...
          data.forEach(function(item, index) {
            ...
            htmlStr += '" data-id="'.concat(
              item[options.id] === undefined ? "" : item[options.id],
              '"'
            );
              ...
              options.fields.forEach(function(field) {
                if (item[field] !== null) {
                  dataHighlight[index][field].forEach(function(value) {
                    var matchStr = tmpItem[field].substr(
                      value.start,
                      value.length
                    );
                    tmpItem[field] = tmpItem[field].replace(
                      new RegExp(matchStr, "i"),
                      '<span class="keyword">'.concat(matchStr, "</span>")
                    );
                  });
                }
              });
            ...
            htmlStr += ' title="'
              .concat(formatParse(options.format, item), '">')
              .concat(formatParse(options.format, tmpItem), "</li>");
          });
          htmlStr += "</ul>";
        } //create dom
      ...
      appendHtml: function appendHtml(data) {
        ...
        var $item = $(
          '<div class="'
            .concat(doms.item, '" data-id="')
            .concat(data[options.id], '" title="')
            .concat(formatParse(options.format, data), '"><span>')
            .concat(
              formatParse(SEPARATOR + options.multiField + SEPARATOR, data),
              '</span><a class="'
            )
            .concat(doms.close, '" data-id="')
            .concat(data[options.id], '" href="javascript:;"></a></div>')
        );
        $item.css({
          ...
          "line-height":
            styles.sightHeight - that.props.multiStyle.space * 2 - 2 + "px"
        });
        $item.find("." + doms.close).css({
          top: parseInt(
            (styles.sightHeight - that.props.multiStyle.space * 2 - 2 - 12) / 2
          )
        });
        var lineNum = parseInt(idArr.length / maxLineItem);
        $input.css(
            ...
            if (e.which == KEY.ESC) {
              ...
              if (currentInput !== "") {
                if ($.trim(currentInput) === "") {
                  magicSearch.hideSearchBox(
                    e.which == KEY.BACKSPACE || e.which == KEY.SPACE
                      ? false
                      : undefined
                  );
                  return false;
          ...
          .on("keydown", function(e) {
            ...
            if (e.which == KEY.ESC) {
              ...
              if (selfOptions.multiple) {
                var $last_multi_item = $magicsearch_wrapper.find(
                  "." + doms.items + " ." + doms.item + ":last"
                );
  
                if (selfOptions.showMultiSkin && $_this.val() === "") {
                  magicSearch
                    .deleteData($last_multi_item.attr("data-id"))
                    .hideSearchBox();
                }
              } else {
        ...
        $("body").on("mousedown", function(e) {
          var $target = $(e.target),
            $magicsearch_wrapper = $(this)
              .find(".".concat(doms.wrapper, " .").concat(doms.box, ":visible"))
              .first()
              .parent(),
          ...
          if (
            index !== undefined &&
            !$target.is(
              ".".concat(doms.wrapper, '[data-index="').concat(index, '"] *')
            )
          ) {
